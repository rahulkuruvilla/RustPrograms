...
impl<E: PairingEngine> Keypair<E> {
    ...
    pub fn generate_keypair<R: Rng>(rng: &mut R, srs: SRS<E>) -> Result<Self, SignatureError> {
        ...
        let private_key = PrivateKey {
            sk: a_g2.into_affine(), // needs to be G2Affine
        };
        let a_g1 = srs.h_g1.mul(a); //g_1^{a}
    ...
    fn prove_signature(
        ...
        let pi_2_g1 = hashed_message.mul(self.alpha.into_repr()); // pi_2 = Z^{alpha}
        let pi_4_g1 = hashed_message.mul(self.beta.into_repr());  // pi_4 = Z^{beta}

    ...
    > Node<E, SPOK, SSIG>
{
    ...
    //returns a DKG share (F_k, u_k2, sigma_k, A_k, Y_k)
    pub fn share<R: Rng>(&mut self, rng: &mut R) -> Result<DKGShare<E, SPOK, SSIG>, DKGError<E>> {
        ...
        self.dealer.participant.state = 1;
        Ok(share)
    }
    ...
    pub fn receive_share_and_decrypt<R: Rng>(
        ...
        println!("this share.participant_id={}", participant_id);
        ...
        match (|| -> Result<E::G2Affine, DKGError<E>> {
            ...
            Ok(secret) => {
                ...
                let participant = self
                    ...
                    .ok_or(DKGError::<E>::InvalidParticipantId(participant_id))?;   //error here 
    ...
    pub fn receive_transcript_and_decrypt<R: Rng>(
        ...
        let secret = transcript.pvss_share.y_i[self.dealer.participant.id]
            .mul(self.dealer.private_key_sig.inverse().unwrap().into_repr())
            .into_affine(); // secret key share of participant i
        ...
        self.dealer.accumulated_secret = self.dealer.accumulated_secret + secret; // master secret key
...
mod test {
    ...
    fn test_2_nodes_verify() {
        const NODES: usize = 4; //why 4 here??

        // a in Field (DKG Secret)?
        // SRS = Scrape crs?
        let rng = &mut thread_rng();
        let srs = SRS::<Bls12_381>::setup(rng).unwrap();
    ...
    // doesn't bls use Field elememts rather than group?
    #[test]
    ...
    >(
        ...
        for i in 0..NODES {
            ...
            // append dealer to dealers list - for all nodes
            dealers.push(dealer);
        }
        ...
        // uses a list of nodes, instantiate nodes
        let mut nodes = vec![];
        for i in 0..NODES {
            let degree = dkg_config.degree;
            let node = Node {
        ...
        // Should ignore participant 1, since we modify its share to be bad. - P1 is an adversary 
        for i in 0..NODES {
            let node = &mut nodes[i];
            ...
            for j in 0..NODES {
                ...
                nodes[j]
                    .receive_share_and_decrypt(rng, share.clone())
                    .unwrap();
                //node[j].aggregator.receive_share - maybe or use a general aggregator that each node has
                if i == 0 {
                    nodes[j]
                        .receive_share_and_decrypt(rng, share.clone())
                        .unwrap();

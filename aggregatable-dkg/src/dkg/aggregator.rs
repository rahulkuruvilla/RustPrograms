    ...
    > DKGAggregator<E, SPOK, SSIG>
{
    pub fn receive_share<R: Rng>(
        &mut self,
        rng: &mut R,
        share: &DKGShare<E, SPOK, SSIG>, //dkg share to aggregate
    ) -> Result<(), DKGError<E>> {
        self.share_verify(rng, share)?;
        let transcript = DKGTranscript {
            degree: self.config.degree,
            num_participants: self.participants.len(),
            contributions: vec![(                           //this is a vector of contributions
        ...
        self.transcript = self.transcript.aggregate(&transcript)?; // if ok aggregate transcript into this node's transcript
        Ok(())
    }
